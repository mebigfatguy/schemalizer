...
public class SchemalizerGUIApp extends JFrame
{
				...
				if (JFileChooser.APPROVE_OPTION == chooser.showOpenDialog(SchemalizerGUIApp.this))
				{
					try
					{
						...
						for (int i = 0; i < files.length; i++)
						{
							...
							{
								File[] subXMLFiles = files[i].listFiles(new XMLFileFilter());
								for (int j = 0; j < subXMLFiles.length; j++) 
								{
									if (subXMLFiles[j].isFile())
										openFile(subXMLFiles[j]);
								}
							}
							else
								openFile(files[i]);
	...
	protected boolean closeAllFrames()
	{
		...
		{
			XMLFileFrame f = (XMLFileFrame)frames[i];
			if (!f.isChanged())
				f.dispose();
		}
	...
	protected void generateSchema(XMLFileFrame[] inputFrames, XMLFileFrame xsdFrame)
	{
		try
		{
			...
			{
				XMLFileFrame frame = inputFrames[i];
	...
	protected XMLFileFrame openFile(File f)
		throws IOException
	{
		XMLFileFrame frame;
